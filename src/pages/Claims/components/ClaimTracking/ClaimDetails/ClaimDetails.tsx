...
import ChatIcon from '../../../Chat/AdjusterChat';
...
const ClaimDetails = ({ claim }: Props) => {
  const { documents, handleUploadDocument, handleDownloadDocument, handleDeleteDocument } =
    useClaimDocuments();

  const [isAdjusterChatOpen, setIsAdjusterChatOpen] = useState(false);
  return (
    <>
      <Card className='claim-details-card'>
        <CardHeader
          title={
            <ClaimStatusHeader
              title={claim.vehicle || claim.address || claim.title}
              incidentDate={claim.incidentDate}
            />
          }
          action={
            <Button variant='contained' size='small' onClick={() => setIsAdjusterChatOpen(true)}>
              Adjuster Chat
            </Button>
          }
        />
        ...
        <CardContent>
          ...
          <section className='claim-section'>
            ...
            <div className='claim-grid'>
              <LabelValue label='Claim Number' value={claim.claimNumber} />

              <LabelValue label='Incident' value={claim.incidentType} />

              <LabelValue label='Status' value={claim.status} />

              {claim.vehicle && <LabelValue label='Vehicle' value={claim.vehicle} />}

              {claim.address && <LabelValue label='Property' value={claim.address} />}
            </div>
          </section>
        </CardContent>
      </Card>
      <ChatIcon isOpen={isAdjusterChatOpen} onClose={() => setIsAdjusterChatOpen(false)} />
    </>
  );
};

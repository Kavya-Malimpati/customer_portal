...
import { type Appointment, type Props } from './interface';

const ScheduleInspection = ({ policy }: Props) => {
  const [vehicle, setVehicle] = useState('');
  const [damageType, setDamageType] = useState('');
  const [serviceMethod, setServiceMethod] = useState<'mobile' | 'shop'>('mobile');
  const [date, setDate] = useState('');
  const [slot, setSlot] = useState('');

  const [appointment, setAppointment] = useState<Appointment | null>(null);

  const vehicleOptions = getVehicleOptions(policy);

  const damageOptions = getDamageOptions(policy);

  const handleSchedule = () => {
    if (!vehicle || !damageType || !date || !slot) {
      return;
    }
    ...
    setAppointment({
      ...
      date,
      slot,
      service: getServiceName(policy, serviceMethod),
    });
  };
